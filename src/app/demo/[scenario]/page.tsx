  ...
  return (
    <main className="min-h-screen bg-[#f6f7f9] text-zinc-950">
      ...
      <div className="mx-auto max-w-[1180px] px-5 py-8 sm:px-8 lg:py-10">
        <Link
          href="/demo"
          className="font-mono text-[11px] text-zinc-500 underline decoration-zinc-300 underline-offset-4 hover:text-zinc-950"
        >
          ← All case files
        </Link>
        ...
        <header className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span
                className={`border px-2 py-0.5 text-[11px] font-medium ${OUTCOME_STYLES[scenario.outcome]}`}
              >
                {scenario.outcome}
              </span>
              {scenario.messy ? (
                <span className="border border-zinc-300 bg-white px-2 py-0.5 font-mono text-[9px] uppercase tracking-[0.1em] text-zinc-600">
                  Messy input
                </span>
              ) : null}
            </div>
            <h1 className="mt-3 text-3xl font-semibold tracking-[-0.035em] text-zinc-950">
              {scenario.name}
            </h1>
          ...
          <Link
            ...
            className="brand-button w-fit shrink-0 rounded-lg px-4 py-2.5 text-[13px] font-semibold transition"
          >
            Run this claim live →
          </Link>
        </header>
        ...
        {demoCase.drift ? (
          <div className="mt-6 rounded-lg border border-amber-300 bg-amber-50/70 px-4 py-3.5">
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-amber-900">
              Seeded evidence has aged · showing {demoCase.drift.actual}, written for{" "}
              {demoCase.drift.expected}
            </p>
            <p className="mt-1.5 max-w-3xl text-[13px] leading-6 text-zinc-700">
        ...
        >
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,360px)]">
            <blockquote className="rounded-lg border border-zinc-200 bg-white px-4 py-3.5 text-[14px] leading-7 text-zinc-800">
              &ldquo;{scenario.rawText}&rdquo;
            </blockquote>

            <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white">
              <p className="border-b border-zinc-100 bg-zinc-50/70 px-4 py-2.5 text-[13px] font-semibold text-zinc-900">
              ...
              <dl className="divide-y divide-zinc-100">
                {[
                  ...
                  >
                    ...
                    <dd className="font-mono text-[11px] leading-5 text-zinc-900">
                      {value}
                    </dd>
                  </div>
                ))}
              </dl>
              <p className="border-t border-zinc-100 bg-zinc-50/60 px-4 py-2.5 text-[11px] leading-5 text-zinc-600">
                {parse.note}
              </p>
            </div>
          </div>
        ...
        >
          <div className="rounded-lg border border-zinc-200 bg-white px-4 py-3.5">
            <p className="max-w-3xl text-[14px] leading-7 text-zinc-800">
              {demoCase.resolutionMessage}
            </p>
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-zinc-100 pt-2.5 font-mono text-[10px] uppercase tracking-[0.12em] text-zinc-400">
              <span>
          ...
          {demoCase.clarifyingQuestion ? (
            <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50/50 px-4 py-3.5">
              ...
              <p className="mt-1.5 text-[14px] leading-6 text-zinc-800">
        ...
        <nav className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-zinc-200 pt-5">
          ...
          <Link
            href={`/demo/${next.key}`}
            className="font-mono text-[11px] text-zinc-600 underline decoration-zinc-300 underline-offset-4 hover:text-zinc-950"
          >
            Next case: {next.name} →
          </Link>
        </nav>

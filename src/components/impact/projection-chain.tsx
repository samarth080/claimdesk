    ...
    <div>
      ...
      <div className="mt-1.5 flex overflow-hidden rounded-lg border border-zinc-300 bg-white focus-within:border-[#f37021] focus-within:ring-1 focus-within:ring-[#f37021]">
        ...
        <input
          ...
          className="min-w-0 flex-1 px-2.5 py-2 font-mono text-body tabular-nums text-zinc-950 outline-none"
        />
        {suffix ? (
          <span className="border-l border-zinc-200 px-2.5 py-2 text-mini text-zinc-500">
            {suffix}
          </span>
        ) : null}
      </div>
    ...
    >
      <div className="flex flex-wrap items-baseline justify-between gap-3 border-b border-zinc-200 bg-zinc-50 px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 text-mini text-zinc-500">
        ...
        <button
          type="button"
          onClick={onClose}
          className="text-detail font-medium text-zinc-500 underline decoration-zinc-300 underline-offset-4 hover:text-zinc-950"
        >
          Close list
        </button>
      </div>
      ...
      <div className="max-h-[26rem] overflow-auto">
        <table className="w-full min-w-[760px] border-collapse text-left">
          <thead className="sticky top-0 border-b border-zinc-200 bg-white font-mono text-micro uppercase tracking-[0.14em] text-zinc-400">
            <tr>
            ...
            {filtered.map((claim) => (
              <tr key={claim.id} className="hover:bg-zinc-50/70">
                <td className="px-4 py-2.5">
                  ...
                  <p className="mt-0.5 max-w-xs truncate text-mini text-zinc-500">
                    {claim.rawText}
                  </p>
                </td>
                <td className="px-4 py-2.5">
                  ...
                  <p className="mt-0.5 font-mono text-micro text-zinc-400">
                    {claim.confidence === null
                      ? "—"
                      : `${Math.round(claim.confidence * 100)}%`}
                  </p>
                </td>
                <td className="px-4 py-2.5 text-mini text-zinc-600">
                  {claim.retailerName ?? "not matched"}
                </td>
                <td className="px-4 py-2.5 text-right font-mono text-mini tabular-nums text-zinc-700">
                  {claim.claimedOrderValue === null
                    ? "—"
                    : formatRupees(claim.claimedOrderValue)}
                </td>
                <td className="px-4 py-2.5 text-right font-mono text-micro tabular-nums text-zinc-500">
  ...
  return (
    <section aria-labelledby="projection-title">
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 border-b border-zinc-200 pb-2.5">
        ...
        {changed ? (
          <button
            ...
            className="font-mono text-micro uppercase tracking-[0.12em] text-zinc-500 underline decoration-zinc-300 underline-offset-4 hover:text-zinc-950"
          >
            Reset defaults
          </button>
        ) : null}
            ...
            >
              ...
              <p
                className={`mt-2 font-mono tabular-nums ${
                  isCost
                    ? "text-display font-medium text-zinc-950"
                    : "text-headline font-medium text-zinc-800"
                }`}
              >
                {stepValue(step)}
              </p>
              <p className="mt-1 text-detail font-medium text-zinc-800">
                {step.label}
              </p>
              ...
              {clickable ? (
                <button
                  ...
                  className="mt-2 w-fit text-mini font-medium text-[#0b5fc6] underline decoration-blue-200 underline-offset-4 hover:decoration-blue-500"
                >
                  {bucket === step.bucket ? "Hide these claims" : "See these claims"}
                </button>
              ) : null}

              <p className="mt-2.5 border-t border-zinc-100 pt-2.5 text-mini leading-4 text-zinc-500">
                {step.note}
              </p>
            </li>
          );
      ...
      <div className="mt-4 grid gap-4 rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-4 sm:grid-cols-[minmax(0,220px)_minmax(0,220px)_minmax(0,1fr)] sm:items-start">
        ...
        />
        <p className="text-mini leading-5 text-zinc-600 sm:pt-6">

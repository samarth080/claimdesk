...
function ArtifactBody({ artifact }: { artifact: CaseArtifact }) {
  if (artifact.log) {
    return (
      <pre className="overflow-x-auto px-4 py-3 font-mono text-[10px] leading-5 text-zinc-700">
        {artifact.log.join("\n")}
      </pre>
    );
  }
  ...
  return (
    <dl className="divide-y divide-zinc-100">
      {(artifact.fields ?? []).map((field) => (
        ...
        >
          <dt className="font-mono text-[10px] uppercase tracking-[0.1em] text-zinc-400">
            {field.label}
          </dt>
          <dd
...
export function CaseArtifacts({ artifacts }: { artifacts: CaseArtifact[] }) {
  ...
  return (
    <section aria-labelledby="case-artifacts-title">
      ...
      <p className="mt-3 border-l-2 border-amber-400 bg-amber-50/50 px-3 py-2 text-[12px] leading-5 text-zinc-700">
          ...
          >
            <figcaption className="border-b border-zinc-100 bg-zinc-50/70 px-4 py-2.5">
              <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
                ...
                <span className="border border-zinc-300 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-[0.1em] text-zinc-500">
            ...
            <div className="border-t border-zinc-100 bg-zinc-50/60 px-4 py-2.5">
              ...
              <p className="mt-2 flex flex-wrap gap-1.5">
                {artifact.corroborates.map((field) => (
                  <span
                    key={field}
                    className="border border-zinc-200 bg-white px-1.5 py-0.5 font-mono text-[9px] text-zinc-600"
                  >
                    {field}
                  </span>
                ))}

...
export function CasePacket({ packet }: { packet: AgentCasePacket }) {
  return (
    <section
      ...
      className="overflow-hidden rounded-2xl border border-blue-200 bg-blue-50/40 shadow-sm"
    >
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-blue-200 px-5 py-4">
        <div>
          ...
          <h3
            ...
            className="mt-1 text-lead font-semibold text-zinc-950"
          >
            {packet.heading}
          </h3>
        </div>
        ...
        <dl className="grid gap-3 sm:grid-cols-2">
          {[...packet.identityFields, ...packet.transactionFields].map((field) => (
            <div key={field.label} className="min-w-0">
              ...
              <dd className="mt-1 break-words font-mono text-mini leading-5 text-zinc-700">
                {field.value}
              </dd>
            </div>
          ))}
        ...
        <div>
          ...
          <ul className="mt-2 space-y-2">
            {packet.evidenceSummary.map((line) => (
              <li key={line} className="flex gap-2 text-detail leading-5 text-zinc-600">
          ...
          <div className="mt-4 border-l-2 border-blue-400 pl-3">
            <p className="font-mono text-micro uppercase tracking-[0.12em] text-blue-700">
              Requested action
            </p>
      ...
      <details className="border-t border-blue-200 bg-white/60">
        <summary className="cursor-pointer px-5 py-3 font-mono text-micro uppercase tracking-[0.12em] text-zinc-500 hover:text-zinc-800">
          View copy-ready plain text
        </summary>
        <pre className="max-h-80 overflow-auto whitespace-pre-wrap border-t border-blue-100 px-5 py-4 font-mono text-micro leading-5 text-zinc-600">
          {packet.copyText}
        </pre>
      </details>
    </section>
